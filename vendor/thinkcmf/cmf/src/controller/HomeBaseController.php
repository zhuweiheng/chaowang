...
class HomeBaseController extends BaseController
{
    ...
    protected function initialize()
    {
        ...
        //取出公共页面需要的数据
        $subsql = DB::name('PortalTagPost')->where(['status'=>'1'])->field('*,count(*) as idc')->group('tag_id')->order('idc desc')->limit(4)->buildSql();
        $tagHotData = DB::name('PortalTag')->alias('a')->join([$subsql=>'b'],'a.id=b.tag_id')->select();
