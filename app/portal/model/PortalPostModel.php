...
class PortalPostModel extends Model
{
     ...
     */
    public function ralates($keywords){
        static $result = array();
        foreach ($keywords as $k => $v) {
            $arts = DB::name('PortalPost')->order('id')->field('id,post_title,create_time')->where('post_status','1')->whereLike('post_keywords','%'.$v.'%')->select()->toArray();
            if(!empty($arts)){
             $result = array_merge($result,$arts);
            }
        }
        $result = $this->delSameKey($result,'id');
        return $result;
    }
     ...
     * @param str 'id'  键名
     */
    public function delSameKey($arr,$key){
        $tmp_arr = array();
